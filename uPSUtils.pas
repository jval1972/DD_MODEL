...
const
  MODELDEF_KEYWORD_COUNT = 48; // 65;  //*NVDS
  ModelDefLookupTable: array[0..MODELDEF_KEYWORD_COUNT - 1] of TRTab = (
...
var
  ...
  begin
    L := 0;
    J := S[0];
    H := MODELDEF_KEYWORD_COUNT - 1;
    while L <= H do
    begin
      I := (L + H) shr 1;
      SName := ModelDefLookupTable[i].Name;
      if J = SName[0] then
      begin
        if S = SName then
        begin
          CheckReserved := True;
          CurrTokenId := ModelDefLookupTable[I].c;
          Exit;
        end;
        if S > SName then
          L := I + 1
