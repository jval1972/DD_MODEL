...
type
  TSynDDModelHighlighter = class(TSynCustomHighlighter)
  private
...
function TSynDDModelHighlighter.IsIdentChar(AChar: WideChar): Boolean;
var
  i: Integer;
begin
  Result := False;
...
function TSynDDModelHighlighter.IsKeyword(const AKeyword: UnicodeString): Boolean;
var
...
function TSynDDModelHighlighter.IsWordBreakChar(AChar: WideChar): Boolean;
begin
  Result := inherited IsWordBreakChar(AChar) and not IsIdentChar(AChar);
end;

constructor TSynDDModelHighlighter.Create(AOwner: TComponent);
begin
...
destructor TSynDDModelHighlighter.Destroy;
begin
  fKeyWords.Free;
  inherited Destroy;
end; { Destroy }

procedure TSynDDModelHighlighter.AnsiProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDModelHighlighter.PasStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDModelHighlighter.CStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynDDModelHighlighter.AsciiCharProc;
begin
...
procedure TSynDDModelHighlighter.BraceOpenProc;
begin
  fTokenID := tkComment;
  fRange := rsPasStyle;
  inc(Run);
...
procedure TSynDDModelHighlighter.PointCommaProc;
begin
  inc(Run);
  fTokenID := tkSymbol;
end;

procedure TSynDDModelHighlighter.CRProc;
begin
  fTokenID := tkSpace;
  Inc(Run);
...
procedure TSynDDModelHighlighter.IdentProc;
begin
...
procedure TSynDDModelHighlighter.IntegerProc;
...
procedure TSynDDModelHighlighter.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynDDModelHighlighter.NullProc;
begin
  fTokenID := tkNull;
  inc(Run);
end;

procedure TSynDDModelHighlighter.NumberProc;
...
procedure TSynDDModelHighlighter.RoundOpenProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
end;

procedure TSynDDModelHighlighter.SlashProc;
begin
  Inc(Run);
  if FLine[Run] = '/' then
  begin
...
procedure TSynDDModelHighlighter.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TSynDDModelHighlighter.StringProc;
var
   delim : WideChar;
begin
  fTokenID := tkString;
...
procedure TSynDDModelHighlighter.UnknownProc;
begin
  inc(Run);
  fTokenID := tkUnknown;
end;

procedure TSynDDModelHighlighter.Next;
begin
  fTokenPos := Run;
  case fRange of
    rsAnsi: AnsiProc;
...
function TSynDDModelHighlighter.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynDDModelHighlighter.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TSynDDModelHighlighter.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TSynDDModelHighlighter.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;
...
// GetCharBeforeToken
//
function TSynDDModelHighlighter.GetCharBeforeToken(offset : Integer = -1) : WideChar;
begin
...
// GetCharAfterToken
//
function TSynDDModelHighlighter.GetCharAfterToken(offset : Integer = 1) : WideChar;
begin
   Result := FLine[fTokenPos+offset];
end;

function TSynDDModelHighlighter.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynDDModelHighlighter.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

procedure TSynDDModelHighlighter.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynDDModelHighlighter.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TSynDDModelHighlighter.SetKeyWords(const Value: TUnicodeStrings);
var
  i: Integer;
begin
  if Value <> nil then
...
class function TSynDDModelHighlighter.GetLanguageName: string;
begin
  Result := 'DDModel';
end;

{$IFNDEF SYN_CLX}
function TSynDDModelHighlighter.LoadFromRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
...
function TSynDDModelHighlighter.SaveToRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
  r := TBetterRegistry.Create;
...
function TSynDDModelHighlighter.GetStringDelim: TStringDelim;
begin
  Result := fStringDelim;
end;

procedure TSynDDModelHighlighter.SetStringDelim(const Value: TStringDelim);
begin
   fStringDelim := Value;
end;

function TSynDDModelHighlighter.GetIdentifierChars: UnicodeString;
begin
  Result := fIdentChars;
end;

procedure TSynDDModelHighlighter.SetIdentifierChars(const Value: UnicodeString);
begin
  fIdentChars := Value;
end;

function TSynDDModelHighlighter.StoreIdentChars: Boolean;
begin
   Result := (fIdentChars <> cDefaultIdentChars);
end;

procedure TSynDDModelHighlighter.SetDetectPreprocessor(Value: boolean);
begin
...
class function TSynDDModelHighlighter.GetFriendlyLanguageName: UnicodeString;
begin
  Result := 'DelphiDoom ModelScript';
end;

// IsStringDelim
//
function TSynDDModelHighlighter.IsStringDelim(aChar : WideChar) : Boolean;
begin
...
initialization
{$IFNDEF SYN_CPPB_1}
  RegisterPlaceableHighlighter(TSynDDModelHighlighter);
{$ENDIF}
end.
